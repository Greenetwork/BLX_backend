...
pub fn get_apn_list(id_list: Vec<i64>, db: &mut Connection) -> Result<Option<APN>, Error> {
    let apn_list = id_list.iter().join(", ");
    ...
    let statement = db
        .prepare(format!(r#"
            ...
            dwr_revise, region, apn_chr, county, crop2016, id, acres,
            ST_AsText(geometry) as geometry
            FROM blx_consolidated_apn WHERE apn IN ({apn_list})
            "#, apn_list = apn_list)
        )?;
